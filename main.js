...
/* ==== SMARTModalPatch (append-only, safe) ==================================
   - Keeps all your original functions intact.
   - Ensures research modal: LEFT media/caption/credit, RIGHT details/members.
   - Ensures team bio modal: LEFT image, RIGHT bio + bold green links.
   - Supports Research Hub `.open-modal-btn[data-modal-target="<personId>"]` clicks.
============================================================================= */
(() => {
  ...
  const safeUrl = (u) => {
    try { const x = new URL(String(u), window.location.origin); return ['http:','https:'].includes(x.protocol) ? x.href : '#'; }
    catch { return '#'; }
  };
  const makeMedia = (url) => {
    if (!url) return null;
    ...
    if (/\.(png|jpe?g|webp|gif|svg)(\?.*)?$/i.test(u)) {
      const img = document.createElement('img');
      img.src = safeUrl(u); img.alt=''; img.loading='lazy';
      img.className='w-full h-auto rounded-md object-cover border border-primary-dark';
      return img;
    }
    if (u.includes('youtube.com/embed/')) {
      const box = document.createElement('div');
      box.className = 'relative w-full'; box.style.paddingBottom = '56.25%';
      const iframe = document.createElement('iframe');
      iframe.src = safeUrl(u); iframe.loading='lazy'; iframe.allowFullscreen=true;
      iframe.setAttribute('allow','accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture');
      iframe.className='absolute top-0 left-0 w-full h-full rounded-md border border-primary-dark';
    ...
    v.className='w-full h-auto rounded-md border border-primary-dark';
    const s = document.createElement('source'); s.src = safeUrl(u); s.type='video/mp4';
    v.appendChild(s); return v;
  };

  // ——— Research modal (uses pre-defined modal DOM in index.html if present) ———
  function openResearchTwoColumn(item) {
    // Prefer your static modal shell if it exists
    const shell = $id('research-description-modal');
    if (shell) {
      // LEFT
      const mediaBox = $id('research-modal-media'); if (mediaBox) { mediaBox.innerHTML=''; const m = makeMedia(item?.modalMedia || item?.image); if (m) mediaBox.appendChild(m); }
      const cap = $id('research-modal-caption'); if (cap) cap.textContent = item?.modalMediaCaption || '';
      const cred = $id('research-modal-credit'); if (cred) {
        let creditText = '';
        if (item?.modalMediaCreditId != null) {
          const team = (Array.isArray(window.teamData)?window.teamData:(window.teamData?.items||[]));
          const alumni = (Array.isArray(window.alumniData)?window.alumniData:(window.alumniData?.items||[]));
          const person = [...team, ...alumni].find(p => String(p.id) === String(item.modalMediaCreditId));
          creditText = person ? person.name : String(item.modalMediaCreditId);
        }
        cred.textContent = creditText ? `Credit: ${creditText}` : '';
      }
      // RIGHT
      const title = $id('research-modal-title'); if (title) title.textContent = item?.title || '';
      const desc = $id('research-modal-description'); if (desc) desc.textContent = String(item?.description || '');
      const members = $id('research-modal-team-members');
      if (members) {
        members.innerHTML = '';
        const ids = Array.isArray(item?.teamMembers) ? item.teamMembers : [];
        if (ids.length) {
          const team = (Array.isArray(window.teamData)?window.teamData:(window.teamData?.items||[]));
          const alumni = (Array.isArray(window.alumniData)?window.alumniData:(window.alumniData?.items||[]));
          const row = document.createElement('div'); row.className='mt-2 flex flex-wrap gap-2';
          const strong = document.createElement('strong'); strong.textContent='Associated Members:';
          members.appendChild(strong); members.appendChild(row);
          ids.forEach(id => {
            const p = [...team, ...alumni].find(x => String(x.id)===String(id));
            const btn = document.createElement('button');
            btn.className='text-primary font-bold hover:underline open-modal-btn';
            btn.setAttribute('data-modal-target', String(id));
            btn.textContent = p ? p.name : String(id);
            row.appendChild(btn);
          });
        }
      }
      shell.classList.add('active'); shell.setAttribute('aria-hidden','false'); shell.focus?.();
      return;
    }

    // Fallback: dynamic overlay (if shell isn't present)
    const overlay = document.createElement('div');
    overlay.className='modal-overlay active'; overlay.setAttribute('role','dialog'); overlay.setAttribute('aria-modal','true');
    const panel = document.createElement('div');
    panel.className='modal-content flex flex-col md:flex-row items-start gap-8';
    const closeBtn = document.createElement('button');
    closeBtn.className='modal-close'; closeBtn.textContent='×'; closeBtn.addEventListener('click', ()=>{ overlay.classList.remove('active'); overlay.remove(); });
    panel.appendChild(closeBtn);

    const left = document.createElement('div'); left.className='w-full md:w-1/2';
    const m = makeMedia(item?.modalMedia || item?.image); if (m) left.appendChild(m);
    if (item?.modalMediaCaption) { const p=document.createElement('p'); p.className='text-xs text-medium-text mt-2'; p.textContent=item.modalMediaCaption; left.appendChild(p); }
    if (item?.modalMediaCreditId != null) { const p=document.createElement('p'); p.className='text-xs text-medium-text'; p.textContent='Credit: '+String(item.modalMediaCreditId); left.appendChild(p); }

    const right = document.createElement('div'); right.className='w-full md:w-1/2 text-left';
    const h = document.createElement('h3'); h.className='text-2xl font-bold text-light-text'; h.textContent=item?.title || '';
    const d = document.createElement('p'); d.className='mt-2 text-sm text-medium-text'; d.textContent=String(item?.description || '');
    right.append(h,d);

    if (Array.isArray(item?.teamMembers) && item.teamMembers.length) {
      const row = document.createElement('div'); row.className='mt-3 flex flex-wrap gap-2';
      const strong = document.createElement('strong'); strong.textContent='Associated Members:'; right.appendChild(strong);
      const team = (Array.isArray(window.teamData)?window.teamData:(window.teamData?.items||[]));
      const alumni = (Array.isArray(window.alumniData)?window.alumniData:(window.alumniData?.items||[]));
      item.teamMembers.forEach(id => {
        const p = [...team, ...alumni].find(x => String(x.id)===String(id));
    ...
    overlay.appendChild(panel);
    (document.getElementById('modal-container') || document.body).appendChild(overlay);
    overlay.classList.add('active');
  }

  // ——— Team/Alumni bio modal (dynamic overlay) ———
  function openPersonTwoColumn(person) {
    const overlay = document.createElement('div');
    overlay.className='modal-overlay active'; overlay.setAttribute('role','dialog'); overlay.setAttribute('aria-modal','true');
    const panel = document.createElement('div');
    panel.className='modal-content flex flex-col md:flex-row items-start gap-8';
    ...
    if (person?.image) {
      const img = document.createElement('img');
      img.src = safeUrl(person.image); img.alt = person?.name || 'Profile photo';
      img.className='w-full h-auto rounded-md object-cover border border-primary-dark';
      left.appendChild(img);
    }

    // Right: name/role/bio + links
    const right = document.createElement('div'); right.className='w-full md:w-1/2';
    const h = document.createElement('h3'); h.className='text-2xl font-bold text-light-text'; h.textContent = person?.name || '';
    right.appendChild(h);
    if (person?.role) { const p = document.createElement('p'); p.className='text-sm text-medium-text mt-1'; p.textContent=person.role; right.appendChild(p); }
    if (person?.bio) { const p = document.createElement('p'); p.className='text-sm text-medium-text mt-3'; p.textContent=person.bio; right.appendChild(p); }

    const links = [];
    if (Array.isArray(person?.links)) person.links.forEach(l => l?.label && l?.url && links.push(l));
    ...
    if (links.length) {
      const row = document.createElement('div'); row.className='mt-4 flex flex-wrap gap-3';
      links.forEach(({label,url}) => {
        const a = document.createElement('a');
        a.href = safeUrl(url); a.target='_blank'; a.rel='noopener';
  ...
  // ——— Delegated clicks; works alongside your existing handlers ———
  document.addEventListener('click', (e) => {
    const t = e.target;

    // Close clicks
    if (t.classList?.contains('modal-overlay')) { t.classList.remove('active'); t.setAttribute('aria-hidden','true'); t.remove(); return; }
    const close = t.closest?.('.modal-close,[data-close-modal]');
    if (close) { const ov = close.closest('.modal-overlay'); if (ov) { ov.classList.remove('active'); ov.remove(); } return; }

    // Research modal: my trigger format
    const r1 = t.closest?.('[data-modal-target="open-research-modal"]');
    if (r1) {
      e.preventDefault();
      const id = r1.getAttribute('data-id') || r1.getAttribute('data-target-id') || r1.dataset.id;
      const items = Array.isArray(window.researchData) ? window.researchData : (window.researchData?.items||[]);
      const item = items.find(x => String(x.id) === String(id) || String(x.title) === String(id));
      if (item) openResearchTwoColumn(item);
      return;
    }

    // Team bio: my trigger format
    const p1 = t.closest?.('[data-modal-target="open-person-bio"]');
    if (p1) {
      e.preventDefault();
      const id = p1.getAttribute('data-id');
      ...
      const person = ds.find(x => String(x.id) === String(id));
      if (person) openPersonTwoColumn(person);
      return;
    }

    // Research Hub “Bio” buttons
    const hub = t.closest?.('.open-modal-btn');
    if (hub && hub.hasAttribute('data-modal-target')) {
      e.preventDefault();
      const id = hub.getAttribute('data-modal-target');
      const ds = (Array.isArray(window.teamData)?window.teamData:(window.teamData?.items||[]))
               .concat(Array.isArray(window.alumniData)?window.alumniData:(window.alumniData?.items||[]));
      const person = ds.find(x => String(x.id) === String(id));
      if (person) openPersonTwoColumn(person);
